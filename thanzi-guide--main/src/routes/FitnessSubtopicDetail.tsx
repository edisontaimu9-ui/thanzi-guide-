...
import { getFitnessSubtopicBySlug, FitnessSubtopicDoc } from '@/lib/fitnessSubtopics';

type Status = 'loading' | 'idle' | 'error';

export function FitnessSubtopicDetail() {
  const { topicSlug, subtopicSlug } = useParams<{ topicSlug: string; subtopicSlug: string }>();
  const [subtopic, setSubtopic] = useState<FitnessSubtopicDoc | null | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    setStatus('loading');
    getFitnessSubtopicBySlug(subtopicSlug)
      .then((result) => {
        setSubtopic(result);
        setStatus('idle');
      })
  ...
  return (
    <main className="mx-auto max-w-2xl px-6 py-12">
      <Link to={`/fitness/${topicSlug}`} className="text-sm text-brand-500 underline dark:text-brand-100">
        ← Back
      </Link>
