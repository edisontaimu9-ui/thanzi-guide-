...
export function ArticleDetail() {
  ...
  return (
    <main className="mx-auto max-w-2xl px-6 py-12">
      <Link to="/learn" className="text-sm text-brand-500 underline dark:text-brand-100">
        ← Back to Learn
      </Link>
      ...
      {status === 'error' && (
        <p className="mt-6 text-sm text-clay-500 dark:text-clay-400">Couldn't load this article right now.</p>
      )}
      ...
      {status === 'idle' && article && (
        <article>
          <h1 className="mt-6 font-display text-3xl text-brand-700 dark:text-sand-100">{article.title}</h1>
          {article.summary && <p className="mt-3 text-brand-500 dark:text-brand-100">{article.summary}</p>}
