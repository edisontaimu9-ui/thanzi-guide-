...
import { listFitnessTopics, FitnessTopicDoc } from '@/lib/fitnessTopics';
import { listSubtopicsForFitnessTopic, FitnessSubtopicDoc } from '@/lib/fitnessSubtopics';

type Status = 'loading' | 'idle' | 'error';

export function FitnessTopicDetail() {
  const { topicSlug } = useParams<{ topicSlug: string }>();
  const [topic, setTopic] = useState<FitnessTopicDoc | null | undefined>(undefined);
  const [subtopics, setSubtopics] = useState<FitnessSubtopicDoc[]>([]);
  ...
  useEffect(() => {
    if (!topicSlug) return;
    setStatus('loading');
    Promise.all([listFitnessTopics(), listSubtopicsForFitnessTopic(topicSlug)])
      .then(([topics, subs]) => {
  ...
  return (
    <main>
      ...
      {status === 'idle' && !topic && (
        <div className="mx-auto max-w-3xl px-6 py-12">
          <Link to="/fitness" className="text-sm text-brand-500 underline dark:text-brand-100">
            ← Back to Fitness
          </Link>
        ...
        <>
          <section className="bg-brand-500 px-6 py-12 text-white">
            <div className="mx-auto max-w-3xl">
              <Link to="/fitness" className="text-sm text-white/80 underline hover:text-white">
                ← Fitness
              </Link>
                ...
                {subtopics.map((sub) => (
                  ...
                  >
                    ...
                    <div className="p-5">
                      ...
                      <Link
                        to={`/fitness/${topic.slug}/${sub.slug}`}
