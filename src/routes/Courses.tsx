...
export function Courses() {
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-12">
      <h1 className="font-display text-3xl text-brand-700">Courses</h1>
      ...
      <div className="mt-8">
        ...
        {status === 'error' && <p className="text-sm text-clay-500">Couldn't load courses right now.</p>}

        {status === 'idle' && courses.length === 0 && (
          <div className="rounded-lg border border-brand-100 p-8 text-center text-brand-500">
            <p className="font-medium text-brand-700">No courses yet</p>
          ...
          <ul className="space-y-3">
            {courses.map((course) => (
              <li key={course.$id}>
                ...
                >
                  <p className="font-display text-lg text-brand-700">{course.title}</p>
                  {course.description && <p className="mt-1 text-sm text-brand-500">{course.description}</p>}
                </Link>
              </li>
            ))}
          </ul>
